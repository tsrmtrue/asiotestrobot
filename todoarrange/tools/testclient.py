...
def generate_mac(count):
    global g_list_mac_address
    for i in range(count):
        g_list_mac_address.append("mac%(i)d"%(locals()))
...
def start_connection():
    ...
    for mac in g_list_mac_address:
        ...
        client.connect(("192.168.1.102",9090))
        client.setblocking(True)
        print  "new client",client
        g_map_logining_peer[client] = [mac,0]
...
def do_login():
    ...
    for client in g_map_logining_peer.keys():
        ...
        if recvstr.find('"errcode":1') <> -1:
            ...
            client.send(build_SCreateReq_str("bb"+g_map_logining_peer[client][0], 100001))
            recvstr = client.recv(1024)
