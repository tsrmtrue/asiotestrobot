...
def parseAndBuild(hfile, moduleName="", protocolEnum="EProtocolId", relativePath=""):
    ...
    fun_addAssignment()
    fun_buildPyStruct_namedlist("%(relativePath)sautotest_client/test_namedlist%(moduleName)s.py"%(locals()))
    fun_buildCodingFun_namedlist("%(relativePath)sautotest_client/test_namedlist_coding%(moduleName)s"%(locals()), "0_001", "test_namedlist%(moduleName)s"%(locals()))
    fun_test_print_all()


    w = open('%(relativePath)sautotest_client/protocol_enums_%(moduleName)s.py'%(locals()), 'w')
