...
def generate_mac(count, prefix, start=0):
...
def start_connection():
    ...
    for mac in g_list_mac_address:
        ...
        client.connect(("120.31.136.164",9091))
    ...
    for client in g_list_authenticated_peer:
        ...
        if len(g_list_authenticated_peer) > i + validCount:
            ...
            try:
                ...
                """
                gmReq = SGmcmdReq()
                gmReq.cmd="setlevel 67"
                sendbuf,l = getSendbuf(C_2_S_GmCmd_Req, gmReq)
                client.send(sendbuf)
